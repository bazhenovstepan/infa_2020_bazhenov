...
def msg_image_select(m):
    ...
    if text == 'Mickey':  # send the appropriate image based on the reply to the "/getImage" command
        img_mickey = BytesIO(urllib.request.urlopen(url1).read())
        bot.send_photo(cid, img_mickey, 
                       reply_markup=hideBoard)  # send file and hide keyboard, after image is sent
        userStep[cid] = 0  # reset the users step back to 0
    elif text == 'Minnie':
        img_minnie = BytesIO(urllib.request.urlopen(url2).read())
        bot.send_photo(cid, img_minnie, reply_markup=hideBoard)
        userStep[cid] = 0
    else:
